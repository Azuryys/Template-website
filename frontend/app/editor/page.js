...
export default function EditorPage() {
  ...
  const handleSaveHotkey = () => {
    ...
    for (const [action, hotkey] of Object.entries(hotkeys)) {
      ...
      if (hotkeyToString(hotkey) === newHotkeyString) {
    ...
    if (conflictingAction) {
      ...
      setHotkeyError(`This hotkey is already used for "${actionLabels[conflictingAction]}". Please choose a different hotkey.`);
      return;
    }
  ...
  }, [hotkeys, selectedObject, canvas, handleMirrorImage]);
